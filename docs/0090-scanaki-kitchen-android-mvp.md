...
4. Redirects successful staff login and other staff routes back to `/kitchen`.
5. Allows HTTPS navigation only on `scanaki.uk` and blocks external WebView navigation.
6. Disables file/content access, third-party cookies, mixed content and WebView permission requests.
7. Allows KDS notification audio without requiring an extra tap.
8. Shows a native offline screen when internet access is lost.
9. Automatically reloads the authenticated KDS when connectivity returns.
