...
| 2026-07-22 | Delta pass for Scanaki Delivery public create + `public_order_token` pay, marketplace delivery webhooks, courier fulfillment IDOR, and SaaS paywall middleware / platform Checkout. Clarified: still **no** inbound Stripe/Revolut *payment* webhooks; delivery ingest webhooks exist. Linked regression tests and residual risks. **Not a penetration test.** |
...
| 2026-07-23 | Delta for **#304**: Public Scanaki Delivery create item IDs may be `TenantProduct.id` (resolved to linked `Product`) or legacy `Product.id`, both tenant-bound; residual note on dual ID space; cite `test_public_create_accepts_tenant_product_menu_ids`. **Not a penetration test.** |
