...
So the base URL (e.g. `https://scanaski.uk`) must be registered. If Revolut validates by path prefix, use e.g. `https://scanaski.uk/menu/` and `http://localhost:4202/menu/`.
...
- **`PUBLIC_APP_BASE_URL`** (required for Revolut redirects)  
  ...
  Examples:
  - Production: `PUBLIC_APP_BASE_URL=https://scanaski.uk`
