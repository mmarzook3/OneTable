    ...
    <div class="auth-page">
      <div class="auth-card">
        <div class="auth-header">
          <h1>{{ 'CUSTOMER_AUTH.TITLE' | translate }}</h1>
          <p>{{ 'CUSTOMER_AUTH.SUBTITLE' | translate }}</p>
        </div>

        <form [formGroup]="form" (ngSubmit)="onSubmit()" data-testid="customer-login-form">
          ...
          @if (error()) {
            <div class="error-banner" data-testid="customer-login-error">{{ error() }}</div>
          }
          <button type="submit" class="btn-submit" [disabled]="form.invalid || loading()" data-testid="customer-login-submit">
...
export class CustomerLoginComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    this.api.customerLogin(email, password).subscribe({
      next: () => void this.router.navigate(['/customer']),
      error: (err) => {
